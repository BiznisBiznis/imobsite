...
router.get('/stats', async (req, res) => {
    try {
        const [[totalVisitors]] = await db.query('SELECT COUNT(*) as count FROM analytics_logs');
        const [[uniqueVisitors]] = await db.query('SELECT COUNT(DISTINCT ip_address) as count FROM analytics_logs');
        const [[topCountryResult]] = await db.query('SELECT country, COUNT(*) as count FROM analytics_logs GROUP BY country ORDER BY count DESC LIMIT 1');
...
router.get('/logs', async (req, res) => {
    try {
        const [logs] = await db.query('SELECT * FROM analytics_logs ORDER BY timestamp DESC');
...
router.post('/track', async (req, res) => {
    try {
        ...
        const ip = req.ip;
        await db.query('INSERT INTO analytics_logs (page, ip_address, country) VALUES (?, ?, ?)', [page, ip, country]);
